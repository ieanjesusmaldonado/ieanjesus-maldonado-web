/**
 * =============================================================================
 * IEANJESÚS MALDONADO — Feed iCalendar: Calendario de la Iglesia
 * =============================================================================
 * Endpoint serverless compatible con Google Calendar, Apple Calendar e iCalendar (RFC 5545).
 * Genera dinámicamente el feed .ics de las actividades principales de la iglesia.
 * =============================================================================
 */
...
// Catálogo aprobado de contingencia (en caso de corte temporal de red con Supabase)
...
exports.handler = async (event, context) => {
    ...
    if (fetchFn) {
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data) && data.length > 0) {
          events = data.filter(e => e.public !== false && e.include_church_calendar === true);
        }
      }
    }
  } catch (err) {
    console.warn('Usando dataset de contingencia para Calendario de la Iglesia:', err.message);
  }
  ...
  return {
    ...
    headers: {
      ...
      'Content-Disposition': 'inline; filename="calendario-iglesia.ics"',
